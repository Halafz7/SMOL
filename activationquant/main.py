...
parser.add_argument('--alpha', type=float, default=10., help='value to initialize alpha with')
parser.add_argument('--act', type=str, default='pact')
parser.add_argument('--val', dest='val', action='store_true', help='create and use validation set from training set')

# Optimization
parser.add_argument('--epochs', metavar='N', type=int, default=200)
    ...
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm1d):
            ...
            if m.bias is not None:
                weight_params.append(m.bias)
        elif True in [isinstance(m, c) for c in qamodules]:
            weight_params.append(m.alpha)
            if m.s is not None:
                prec_params.append(m.s)
