...
# Model specs in terms of precision and fp training
...
parser.add_argument('--model', type=str, default='preresnet', help='model architecture')
...
parser.add_argument('--lmbda', type=float, default=7e-07)
...
def load_model(num_classes, log):
    if args.model == 'preresnet':
        model = PreResNet(p_init=args.p_init, fp_layers=args.fp_layers, shortcut_type=args.shortcut_type)
    if args.model == 'mobilenet':
        model = MobileNetV2(p_init=args.p_init, fp_layers=args.fp_layers, shortcut_type=args.shortcut_type)
    elif args.model == 'shufflenet':
        model = ShuffleNet(p_init=args.p_init, fp_layers=args.fp_layers, shortcut_type=args.shortcut_type)
...
def main():
    ...
    if args.resume:
        if args.resume == 'auto':        
            rs = glob.glob("{}/checkpoint_*.pth.tar".format(args.save_path))
            idxs = [int(r.split("_")[-1].split(".pth.tar")[0]) for r in rs]
            idxs = sorted(idxs)
            if len(idxs) == 0:
                args.resume = 'None'
            else:
                most_recent = "checkpoint_{}.pth.tar".format(idxs[-1])
                args.resume = os.path.join(args.save_path, most_recent)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
          ...
          'optimizer1' : optimizers[1].state_dict() if len(optimizers) > 1 else None
...
def train(train_loader, model, criterion, optimizers, epoch, log, aux_log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if epoch < args.T and not args.baseline:
