...
class NoisyConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride, bias, p_init):
        ...
        self.weight_s = nn.Parameter(torch.tensor(s))
        if self.bias is not None:
            self.bias_s = nn.Parameter(torch.tensor(s))
